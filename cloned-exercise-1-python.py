...
smallestFuncValue: int = reduce(lambda x, y: x if x < y else y, map(lambda x: x ** 2 - 34 * x + 325, myList))
...
averageTemperatures: dict[str, float] = dict(
    (sensor, average([temp for _, temp in group]))
    for sensor, group in groupby(
        sorted(temperatureReadings, key=lambda x: x[0]),
        key=lambda x: x[0]
    )
)
...
# MAGIC %md
# MAGIC
# MAGIC 1. I have been mosly using PHP, JS and python for more than 4 years, and I have had more than 4 courses at Tampere university on programming tasks.
# MAGIC 2. It was quite easy to use. It somehow remembers me of google colab with it very well-designed and easy-to-use
# MAGIC 3. They were quite interesting and a bit challenging.
# MAGIC
...
# MAGIC %md
# MAGIC
# MAGIC - I just used AI for the task explanation on the average func as I did not totally understant that!
# MAGIC - No, I did it alone on my own
# MAGIC
